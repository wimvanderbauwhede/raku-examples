...
# @sigils
# );
# $RefactorF4Acc::Parser::
$Expressions::defaultToArrays=1;

for  'a1_i* 42 + bbx','z(j+i,k*km)*p(i+1,j+jm)','i+1','v( i + 1 )','z','z(j,k)','j+k','i-im' -> $str_  {
    print "$str_\t"; 
    my $str=$str_;
...
for '1 .and. .not. 0','1 <= 2', '(3.5 < 4) .or. (1 == 0.0)' -> $str {
    print "$str\t";
...
for '*8','RANK ( N, *8, *9 )','f(x)(y)', 'a**b**3', 'B .and. .not. A .or. C','.not.A .and. B','A(I,J,K)(M+N,1)', 'dimension(0:im,-1:jm)','a(1,1)', 'A(1,J,K)(M:N,1)', 'f(x+2,1)(v,12)','time',
